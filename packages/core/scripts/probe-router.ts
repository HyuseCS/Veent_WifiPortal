...
try {
	...
	for (const e of wgHost)
		console.log(
			`  - action=${e.action} dst-host=${e['dst-host'] ?? '*'} comment=${e.comment ?? ''}`
		);
	...
	for (const e of wgIp)
		console.log(
			`  - action=${e.action} dst-address=${e['dst-address'] ?? '*'} comment=${e.comment ?? ''}`
		);
	...
	for (const b of bindings.slice(0, 10))
		console.log(
			`  - ${b['mac-address'] ?? b.address ?? '?'} type=${b.type} comment=${b.comment ?? ''}`
		);
	...
	for (const p of profiles)
		console.log(
			`  ${p.name}: login-by=${p['login-by'] ?? '?'} html-directory=${p['html-directory'] ?? '?'}`
		);
