<script lang="ts">
	...
	// unresolved Sentry issues and it's tracked as a source='sentry' incident — the convenience
	// path so a manager needn't hop to /sentry. ponytail: sentry mode reuses the existing
	// /sentry?/track action (cross-route form post) rather than duplicating it here.
	let {
		open = $bindable(false),
		issue = null,
		staff,
		...
		sentryIssues?: { id: string; shortId: string; title: string; permalink: string }[];
	...
	const sentryOptions = $derived([
		{ value: '', label: 'Select a Sentry issue…' },
		...sentryIssues.map((s) => ({ value: s.id, label: `${s.shortId} — ${s.title}` }))
	]);
	...
	// Picking a Sentry issue prefills the incident title from its title (still editable).
	function onPickSentry() {
		if (selectedSentry) title = selectedSentry.title;
	}
...
<BaseDialog bind:open reset={seed} class="max-w-3xl">
	<div class="flex items-center gap-3">
		...
		<div class="min-w-0">
			...
			<p class="text-xs text-muted">
				{#if isSentry}
					Track an unresolved Sentry error as an assigned incident — it stays in the Sentry feed
				{:else}
	...
	{#if !issue && sentryConfigured}
		<!-- Source toggle — only on a new incident (source is immutable once created). -->
		<div class="mt-4">
			<FilterTabs
				tabs={[
	...
	>
		...
		{#if isSentry && selectedSentry}
			<!-- Snapshot fields read by createIssueFromSentry (id comes from the picker Select below). -->
			...
			<div class="space-y-4">
				{#if isSentry}
					<Select
						...
						bind:value={sentryIssueId}
						onchange={onPickSentry}
						required
					/>
					...
					{:else if selectedSentry}
						<div class="space-y-1 rounded-lg border border-border bg-surface p-3">
							<div class="font-mono text-xs text-muted">{selectedSentry.shortId}</div>
