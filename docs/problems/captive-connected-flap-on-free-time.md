...
> Best-supported hypothesis below; **not yet confirmed by packet capture** — includes a concrete
> verification step. Likely a walled-garden / captive-detector interaction, **not** the free-time
> button itself (which matches the reporter's own uncertainty).
...
1. **Narrow the Google walled-garden entries** so the reCAPTCHA hosts are allowed but the probe
   hosts are not. Practical approach: keep `www.gstatic.com` / `www.google.com` /
   `*.recaptcha.net` as needed by reCAPTCHA, and add explicit **deny/redirect** entries for
   `connectivitycheck.gstatic.com`, `clients3.google.com`, and bare `www.google.com/generate_204`
   ahead of the broad allows (walled-garden rules are ordered — a more specific `action=deny`/
   redirect before the `*.google.com` allow wins). Needs testing that Maya's reCAPTCHA still loads.
