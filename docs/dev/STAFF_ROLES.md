...
| **Finance** — view                                                                      |  ✅   |  ✅   |
| **Sentry** — view                                                                       |  ✅   |  ✅   |
| **Content Management** (packages/tiers, FAQ, session limits) — view _and_ edit          |  ❌   |  ✅   |
| **Staff** (invite, set status, remove, promote, owner-change)                           |  ❌   |  ✅   |

**In short:** admins run day-to-day operations and manage _customer_ accounts (block/unblock/kick);
...
- **Owner change / step-down:** demoting or removing an owner (incl. an owner stepping down) needs
  **unanimous approval from every _other_ owner** — a request/approve flow
...
- **Base gate** — `apps/admin/src/routes/(app)/+layout.server.ts` requires **signed-in + 2FA
  enrolled** for every page in the app shell. It does _not_ gate on role; it just reads the role and
  passes it down.
