...
| ngrok tunnel down | Not a correctness risk: the on-return reconcile (`/top-up/processing`) and cron reconcile (`/api/payments/reconcile`) credit by polling Maya directly. The relay affects **latency**, not correctness. **Reconcile is authoritative** — `getCheckoutStatus` reads the checkout only to get its payment id, then verifies the **payment** endpoint (`GET /payments/v1/payments/{id}`), the same source the webhook trusts. (Previously it read paid-status off the checkout resource, which Maya doesn't reliably flip to paid — so a missed webhook left a paid payment stuck `pending` forever. Fixed.) |
