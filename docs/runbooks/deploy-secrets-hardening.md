# Deploy hardening checklist — secrets & proxy config (L-5 / L-7 / NC-1)

These are **operator/deploy-time** steps that live outside the code (in each app's real `.env` and the
reverse-proxy config). The code enforces what it can (`validateEnv` fails fast on a missing or
too-short `BETTER_AUTH_SECRET` and warns on a risky `ADDRESS_HEADER`), but the following must be set
...
- [ ] **`BETTER_AUTH_SECRET` is 32+ chars, random, per-app.** Generate independently for admin,
      ...
      `validateEnv` now rejects a <32-char secret in production.
