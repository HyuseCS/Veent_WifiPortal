...
to be reviewed *before* any session-minting code lands (per the project rule that auth/session/
...
- **A1 — Co-located freeloader.** Another guest on the same open AP. Can sniff traffic and spoof
  MACs. Goal: spend someone else's credits for free. *Primary actor.*
...
|---|---|---|
| **C1** TLS pre-auth + Secure/HttpOnly/SameSite=Lax cookies, `ORIGIN=https://…`, walled-garden the HTTPS origin | T1 (and the *pre-existing* sidejacking risk) | **Hard prerequisite.** Needs a cert reachable pre-auth on the LAN portal host — a deploy decision (see §8) |
| **C2** Silent (A) gated to low-risk views only | T2 | "Recognized," not "trusted" |
| **C3** OTP step-up for spend/account actions | T2, T5 | Reuses the existing phone-OTP flow; the *credential* check 2b otherwise skips |
...
- **Same-MAC, same-time spoofing of low-risk views.** With C1–C9, A1 who spoofs the victim's MAC
  *while the victim has an active session* can still get a **balance-view** session — but **cannot
...
- **Pre-existing sidejacking until C1 ships.** Until TLS pre-auth lands, the portal cookie is
  sniffable today — independent of 2b. C1 should arguably land *first*, on its own.
...
>   ORIGIN is missing or non-https; `docs/mikrotik/login.html` carries a prod-HTTPS requirement note.
...
2. **Silent re-auth (A) at all, or token-only (B)?** Given the §7 residual, do we accept
   ...
   *all* CNA→browser continuity? (B) removes T2/T4/T5 entirely at the cost of one user tap.
3. **Step-up scope (C3)** — confirm the exact action set behind OTP: spend credits, start tier,
   change account. (Balance + top-up *initiation* stay friction-free; the *charge* is already
   webhook-gated.)
...
1. **C1 (TLS pre-auth + cookie flags) first, standalone** — it fixes a real *current* risk and is
