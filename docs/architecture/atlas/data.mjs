...
export const DECISIONS = [
  { axis: 'Router seam', decision: 'Every router call goes through the `NetworkController` interface. Real provider is `mikrotik.ts`, tests use `stub.ts`. Nothing outside `packages/core/src/integrations/network/` speaks RouterOS.', adr: '—' },
  { axis: 'Walled garden ownership', decision: 'The garden is code-owned. `setup:router` provisions three tagged groups — `veent-admin:probe`, `:payment`, `:portal` — in that order so denies land above allows.', adr: '—' },
  { axis: 'Provisioning safety', decision: 'Default `setup:router` is purely additive. `--reconcile` is an opt-in prune that removes only rows carrying the code tag AND `action=allow`. `--wipe` clears both menus for a hard rebuild.', adr: '—' },
  { axis: 'GCash reachability', decision: 'A `/system scheduler` item (`gcash-resolve`, 5 min) resolves the host and upserts an IP row. Hostname rules cannot follow the CNAME to Akamai. The originally designed DoH/DoT block was proven unnecessary and never shipped.', adr: '—' },
  { axis: 'Payment surface', decision: 'GCash and Maya only. Google Pay is blocked by Android WebView in captive mode; PayMongo and Xendit had no integration code and were pruned.', adr: '—' },
  { axis: 'Browser return URL', decision: '`successUrl`/`cancelUrl` use `event.url.origin` (the walled-gardened LAN portal address). `TUNNEL_ORIGIN` is for the server-to-server webhook only — the guest is still captive at redirect time.', adr: '—' },
  { axis: 'Access grant', decision: 'Paid guests are granted with `ip-binding type=bypassed`, which skips hotspot byte accounting. Per-AP guest throughput is therefore not measurable, by design.', adr: '—' },
  { axis: 'Session→AP binding', decision: '`resolveNetworkIdForMac` resolves the Option-82 circuit-id first, then falls back to interface name. A shared bridge fronting several APs must not capture the session.', adr: '—' },
  { axis: 'MAC trust', decision: 'A fallback-resolved MAC (device cookie, `last_known_mac`) is never treated as a verified binding, and never entrenches `last_known_mac`. The `?mac=` param is client-influenceable by nature.', adr: '—' },
  { axis: 'Auth isolation', decision: 'Two separate `betterAuth()` instances with separate secrets and cookie prefixes — `veent-portal` and `radius-admin`. Never cross-wired.', adr: '—' },
  { axis: 'Schema authority', decision: '`@veent/db` is the sole schema and migration source for all three apps. The dev DB is push-managed, so `db:migrate` drifts — apply DDL directly and still generate the migration file.', adr: '—' },
  { axis: 'Time storage', decision: 'Finance and session columns are `timestamptz` (migration `0052`), with real Manila-day→UTC-instant maths in `period.ts`.', adr: '—' },
  { axis: 'Scheduled work', decision: 'No cron app. Prod hits guarded HTTP endpoints from an external scheduler with an `x-cron-secret` header; `scripts/dev-cron.ts` polls them in dev.', adr: '—' },
];
...
export const NODES = [
  { id: 'G', code: 'G', name: 'Guest device', short: 'GUEST', group: 'edge', gx: 0, gy: 0, w: 2, d: 2, h: 40, kind: 'screen',
    ...
    steps: [['Associate', 'Joins the SSID, gets a DHCP lease.'], ['Probe', 'The OS hits a known URL to test for a captive portal.'], ['Redirect', 'Gets bounced to the portal and the sign-in window opens.'], ['Granted', 'After a grant, the same MAC passes through freely.']],
    cond: [{ q: 'Can the fallback MAC path (unverified banner, reconnect prompt) be reproduced live?', r: 'Not reproduced — it needs live IP→MAC resolution to fail on purpose. Proven by code and unit tests only (2026-07-23).' }] },

  { id: 'MT', code: 'MT', name: 'MikroTik router', short: 'MIKROTIK', group: 'edge', gx: 4, gy: 0, w: 3, d: 3, h: 56, kind: 'slab',
    ...
    steps: [['Hotspot', 'Holds the guest at the login page.'], ['Walled garden', 'Allows only the whitelisted hosts.'], ['Bind', 'Adds an ip-binding row for a granted MAC.'], ['Report', 'Answers health probes and reports Option-82 circuit-ids.']],
    cond: [{ q: 'Should the false-DOWN AP case have a code-level guard, not just a runbook?', r: 'Impossible as designed — online_since/offline_since are current-state stamps, not history. Deferred; the ip-binding runbook is the shipped mitigation (2026-07-21).' }] },

  { id: 'WG', code: 'WG', name: 'Walled garden', short: 'WALLED GDN', group: 'edge', gx: 4, gy: 4, w: 2.5, d: 2.5, h: 30, kind: 'gate',
    ...
    steps: [['Probe denies', 'Deny rows first so they win on a fresh garden.'], ['Payment allows', 'Maya, GCash/Alipay/Mynt, googleapis.'], ['Portal allows', 'Admin and portal origins plus the LAN portal IP.'], ['Reconcile', 'Optional prune of drifted code-owned allow rows.']],
    cond: [{ q: 'Which extra wallets and banks should be onboarded?', r: 'Nine candidates listed UNVERIFIED in docs/mikrotik/walled-garden.md (GoTyme, SeaBank, GrabPay, ShopeePay, Coins.ph, BDO, BPI, Landbank, Security Bank). None whitelisted — each needs the ₱0 recon protocol first (2026-07-30).' }] },

  { id: 'GS', code: 'GS', name: 'gcash-resolve scheduler', short: 'GCASH SCHED', group: 'edge', gx: 1, gy: 5, w: 2, d: 2, h: 34, kind: 'job',
    ...
    steps: [['Tick', 'Fires every 5 minutes.'], ['Resolve', 'Looks up the current edge address.'], ['Upsert', 'Writes the gcash-auto IP row.'], ['Survive wipe', 'Re-adds the row within 5 min of a hard reset.']],
    cond: ['Rule of thumb to keep applying: a host that CNAMEs to a CDN needs a resolve scheduler; a host that resolves to the provider’s own IP needs only a PAYMENT_HOSTS entry.'] },

  { id: 'CU', code: 'CU', name: 'Customer portal', short: 'PORTAL', group: 'portal', gx: 8.5, gy: 0, w: 3, d: 3, h: 50, kind: 'screen',
    ...
    steps: [['Land', 'Captive redirect arrives with the MAC.'], ['Login', 'Phone number, then a one-time code.'], ['Dashboard', 'Shows time left and whether this device is bound.'], ['Top up', 'Starts a Maya checkout.']],
    cond: [] },

  { id: 'OT', code: 'OT', name: 'OTP and SMS', short: 'OTP / SMS', group: 'portal', gx: 4, gy: -4.5, w: 2.5, d: 2.5, h: 38, kind: 'box',
    ...
    steps: [['Send', 'Gateway call, then log the attempt.'], ['Verify', 'Guest types the code back.'], ['Sweep', 'Cron checks Cast delivery receipts.'], ['Prune', 'Rows older than 48h are dropped.']],
    cond: [{ q: 'Is the Cast delivery-receipt response shape stable beyond the one observed REJECTD case?', to: 'Blocked on Cast activating a real sender ID for live traffic' }] },

  { id: 'MY', code: 'MY', name: 'Maya payments', short: 'MAYA', group: 'money', gx: 8.5, gy: -4.5, w: 2.5, d: 2.5, h: 38, kind: 'box',
    ...
    steps: [['Checkout', 'Portal creates the checkout and redirects.'], ['Pay', 'Guest pays on Maya’s page.'], ['Webhook', 'Maya calls the portal server-side.'], ['Reconcile', 'Cron sweeps anything the webhook missed.']],
    cond: [] },

  { id: 'LD', code: 'LD', name: 'Ledgers', short: 'LEDGERS', group: 'money', gx: 18, gy: 0, w: 2.5, d: 2.5, h: 26, kind: 'store',
    ...
    steps: [['Credit', 'A settled payment writes a credit row.'], ['Spend', 'A grant draws it down.'], ['Report', 'Finance reads a merged, deduped activity list.']],
    cond: [] },

  { id: 'SS', code: 'SS', name: 'Sessions and grants', short: 'SESSIONS', group: 'money', gx: 13, gy: 0, w: 3, d: 2.5, h: 46, kind: 'box',
    ...
    steps: [['Resolve device', 'Circuit-id first, then interface name.'], ['Grant', 'Write the session row, call the router.'], ['Pause', 'Freeze the clock during an AP outage.'], ['Revoke', 'Expire the session and unbind the MAC.']],
    cond: [] },

  { id: 'NC', code: 'NC', name: 'NetworkController', short: 'ROUTER SEAM', group: 'shared', gx: 13, gy: 4.5, w: 2.5, d: 2.5, h: 42, kind: 'cards',
    ...
    steps: [['Choose', 'Env picks mikrotik or stub.'], ['Trace', 'Each method is wrapped in a span.'], ['Call', 'Grant, revoke, probe, provision, wipe.'], ['Degrade', 'Timeout becomes a typed, warning-level error.']],
    cond: [] },

  { id: 'DB', code: 'DB', name: 'Postgres', short: 'DATABASE', group: 'shared', gx: 18, gy: 4, w: 3, d: 3, h: 30, kind: 'store',
    ...
    steps: [['Define', 'Schema files under src/schema/.'], ['Generate', 'drizzle-kit writes the migration.'], ['Apply', 'Direct DDL in dev; the runbook in prod.'], ['Share', 'All three apps read the same tables.']],
    cond: [] },

  { id: 'LO', code: 'LO', name: 'Locator map', short: 'LOCATOR', group: 'shared', gx: 8.5, gy: 12.5, w: 2, d: 2, h: 36, kind: 'screen',
    ...
    steps: [['Read', 'Query hotspot locations.'], ['Draw', 'Render Leaflet markers.']],
    cond: [] },

  { id: 'AD', code: 'AD', name: 'Admin dashboard', short: 'ADMIN', group: 'staff', gx: 8.5, gy: 8, w: 3, d: 3, h: 54, kind: 'screen',
    ...
    steps: [['Sign in', 'Password, then TOTP.'], ['Operate', 'Networks, finance, issues, staff, map.'], ['Provision', 'Run setup:router against the live box.'], ['Audit', 'Every mutation writes its own event row.']],
    cond: [{ q: 'Does the manager issue board need row pagination?', to: 'Backlog — lazy event loading shipped; row pagination still open (GH backlog)' }] },

  { id: 'HL', code: 'HL', name: 'AP health and outage', short: 'AP HEALTH', group: 'staff', gx: 13, gy: 8.5, w: 2.5, d: 2.5, h: 40, kind: 'job',
    ...
    steps: [['Probe', 'Ask the router about each AP.'], ['Record', 'Upsert the health row, retry on name collision.'], ['Pause', 'Freeze sessions bound to a down AP.'], ['Resume', 'Unfreeze when it comes back.']],
    cond: [{ q: 'Has the live down-AP case been observed end to end in production?', to: 'Accepted known-gap — tracked in the general-plans backlog' }] },

  { id: 'CR', code: 'CR', name: 'Scheduled jobs', short: 'CRON', group: 'shared', gx: 13, gy: 12.5, w: 2, d: 2, h: 34, kind: 'job',
    ...
    steps: [['Revoke', 'Expire finished sessions.'], ['Reconcile', 'Catch payments the webhook missed.'], ['Sweep', 'Check OTP delivery receipts.'], ['Refresh', 'Re-probe AP health.']],
    cond: ['The five-minute OTP sweep cadence has to be configured on the external prod scheduler — nothing in the repo enforces it.'] },

  { id: 'SN', code: 'SN', name: 'Sentry and incidents', short: 'SENTRY', group: 'staff', gx: 18, gy: 8.5, w: 2.5, d: 2.5, h: 38, kind: 'box',
    ...
    steps: [['Capture', 'Error is caught and scrubbed.'], ['Classify', 'Router timeouts drop to warning level.'], ['Track', 'Staff pull it onto the incident board.'], ['Resolve', 'Assignment and events are audit-logged.']],
    cond: [] },

  { id: 'MR', code: 'MR', name: 'Multi-router support', short: 'MULTI-ROUTER', group: 'off', ghost: true, gx: 0, gy: 9, w: 2.5, d: 2.5, h: 40,
    ...
    steps: [['Credentials', 'Get Fatap AP-API access.'], ['Generalise', 'Make NetworkController multi-instance.'], ['Route', 'Bind each session to its own router.']],
    cond: [{ q: 'When do we get Fatap AP-API credentials?', to: 'GH #100 — plan archived as deferred/revisitable (2026-07-30)' }] },
];

export const FLOWS = [
  { id: 'free', name: 'A guest gets online free', hops: [
  ...
  { id: 'paid', name: 'A guest pays', hops: [
  ...
  { id: 'garden', name: 'Staff rebuild the walled garden', hops: [
  ...
  { id: 'health', name: 'An access point goes down', hops: [
  ...
  { id: 'incident', name: 'An error becomes an incident', hops: [
...
export const CH = [
  { id: 'guest', title: 'A guest and the router', reveal: ['G', 'MT'],
    ...
    flow: [['G', 'MT', 'associate', { mac: 'AA:BB:CC:11:22:33' }], ['MT', 'G', 'held captive', { state: 'unauthorised' }]] },

  { id: 'garden', title: 'The short list of allowed hosts', reveal: ['WG', 'CU'],
    ...
    flow: [['G', 'MT', 'probe', { url: '/generate_204' }], ['MT', 'WG', 'check allow-list', { host: 'portal' }], ['MT', 'CU', 'captive redirect', { to: '/login' }], ['CU', 'G', 'sign-in page', { status: 200 }]] },

  { id: 'who', title: 'Proving who you are', reveal: ['OT'],
    ...
    flow: [['CU', 'OT', 'send code', { phone: '+639••••••123' }], ['OT', 'CU', 'verified', { ok: true }]] },

  { id: 'grant', title: 'Giving out time', reveal: ['SS', 'NC'],
    ...
    flow: [['CU', 'SS', 'grant free time', { minutes: 30 }], ['SS', 'NC', 'allow mac', { mac: 'AA:BB:CC:11:22:33' }], ['NC', 'MT', 'ip-binding', { type: 'bypassed' }], ['MT', 'G', 'internet', { state: 'granted' }]] },

  { id: 'pay', title: 'Paying for more', reveal: ['MY', 'LD'],
    ...
    flow: [['CU', 'MY', 'create checkout', { amount: 20 }], ['MY', 'CU', 'webhook: paid', { status: 'PAYMENT_SUCCESS' }], ['CU', 'LD', 'credit', { credits: 20 }], ['LD', 'SS', 'spend', { minutes: 120 }]] },

  { id: 'store', title: 'Where everything is written', reveal: ['DB', 'LO'],
    ...
    flow: [['SS', 'DB', 'write session', { table: 'network_sessions' }], ['LD', 'DB', 'write ledger', { table: 'credit_ledger' }], ['LO', 'DB', 'read hotspots', { auth: null }]] },

  { id: 'staff', title: 'Staff run the shop', reveal: ['AD'],
    ...
    flow: [['AD', 'NC', 'setup:router --reconcile', { dryRun: true }], ['NC', 'WG', 'prune drifted rows', { removed: 3 }], ['AD', 'DB', 'read finance', { scope: 'unified' }]] },

  { id: 'watch', title: 'Watching the access points', reveal: ['CR', 'HL'],
    ...
    flow: [['CR', 'HL', 'refresh health', {}], ['HL', 'NC', 'probe aps', { count: 6 }], ['NC', 'MT', 'ping', {}], ['MT', 'NC', 'no reply', { rtt: null }], ['HL', 'SS', 'pause clocks', { sessions: 4 }]] },

  { id: 'break', title: 'When things break', reveal: ['SN'],
    ...
    flow: [['CU', 'SN', 'scrubbed error', { pii: 'masked' }], ['SN', 'AD', 'track issue', { verified: true }], ['AD', 'DB', 'issue + event', { tx: true }]] },

  { id: 'later', title: 'The odd corners and what is next', reveal: ['GS', 'MR'],
    ...
    flow: [['GS', 'WG', 'gcash-auto ip', { every: '5m' }], ['WG', 'MT', 'write ip row', { tag: 'gcash-auto' }]] },
