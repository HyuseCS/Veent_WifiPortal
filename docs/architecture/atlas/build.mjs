...
function buildSystemMd() {
	...
	CH.forEach((c, i) =>
		out.push(
			`${i + 1}. **${c.title}** — ${md(c.lede)}${c.reveal.length ? ` _(adds ${c.reveal.join(', ')})_` : ''}`
		)
	);
	out.push('');
	out.push('## Structures', '');
	...
	for (const f of FLOWS) {
		...
		f.hops.forEach((h, i) =>
			out.push(
				`| ${i + 1} | ${h[0]} → ${h[1]} | ${h[2]} | \`${JSON.stringify(h[3]).replace(/\|/g, '\\|')}\` |`
			)
		);
		out.push('');
	}
	...
	index.forEach(([id, code, c]) =>
		out.push(c.r ? `- ~~**${id}**~~ (${code}) ✓ ${md(c.r)}` : `- **${id}** (${code}) ${md(c.q)}`)
	);
	out.push('');
	if (META.platformGives || META.weOwn)
		out.push(
			'## What the platform gives vs what we own',
			'',
			`**Platform gives:** ${META.platformGives || ''}`,
			'',
			`**We own:** ${META.weOwn || ''}`,
			''
		);
	if (META.filesystem)
		out.push('## Planned filesystem', '', '```', META.filesystem.trimEnd(), '```', '');
	out.push(
		'## How this file is maintained',
		'',
		`Generated from \`${META.sourcePath || 'atlas/data.mjs'}\` by \`${META.buildCmd || 'node atlas/build.mjs'}\`, which also builds the interactive atlas (\`atlas.html\`${META.artifactUrl ? `, published at ${META.artifactUrl}` : ''}). Edit the data file, rebuild, republish — never edit this file by hand.`,
		''
	);
	return out.join('\n');
}
...
function buildAtlasHtml() {
	...
	return tpl
		.replace('__TITLE__', META.title + ' Atlas')
		.replace(
			'/*__DATA__*/',
			data +
...
console.log(
	`built SYSTEM.md + atlas.html · ${cnt.open} open · ${cnt.res} resolved · ${NODES.length} structures · ${DECISIONS.length} decisions`
);
