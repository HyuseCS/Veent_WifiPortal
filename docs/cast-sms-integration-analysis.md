...
- Cast is the _easiest_ of the four to wire: static base URL, header auth (`X-API-Key`), E.164
...
Cast is a strictly _simpler_ adapter than the three we already have: no phone reformatting, no
...
If you later want Cast to be the _default_ (so an unset `SMS_PROVIDER` picks Cast), that's a
...
- **Idempotency** (`X-Idempotency-Key` header, 24h replay cache). Would prevent a double-send if a
  login retry re-fires. _Skip:_ better-auth already rate-limits OTP requests, and a rare duplicate
...
- **Sandbox key** (`cast_test_…`) for CI / manual test without spending credits or sending real
  SMS. _Worth grabbing_ for a test environment — same adapter, just a test key; response carries
