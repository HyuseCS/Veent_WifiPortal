...
`.prettierrc` path drift (unrelated to IMS); manager-board pagination (L3 Option 1, row
pagination) remains deferred. IMS e2e spec modernization (the L6a a11y/modal-click rewrite + self-report tile coverage)
...
see `completed/`. Manager-board lazy event timeline (L3 Option 2) shipped 22-07-26 —
`completed/manager-board-lazy-events_22-07-26/` — board no longer eager-loads every issue's event
history; fetches lazily per-issue on row-expand via the existing `/issues/[id]/detail` endpoint.
See `process/features/incident-management/backlog/` for the remaining filed notes.
