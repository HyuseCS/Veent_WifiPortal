...
- [x] **2.6** AP row upsert keyed on `mac` (`onConflictDoUpdate(target: networkHealth.mac)`), writing name/online/wanOk/users/latencyMs/apCircuitId/attributionSource/lastSampleAt + since-transitions. Name-collision edge: catch the `network_health_name_key` unique violation and retry once with `${hostname} (${mac.slice(-5).replace(':','')})`; hostname-less AP names as `AP ${mac}`. **DONE 20-07-26** — the retry-on-collision half shipped separately as its own plan: `process/general-plans/completed/ap-name-collision-retry_20-07-26/ap-name-collision-retry_PLAN_20-07-26.md` (`isNameUniqueViolation` + once-retry in `upsertApRow`, `packages/core/src/services/networkHealth.ts`, commit `9988faf`). What shipped in the original 4d67e76 commit was only the `resolveApName` pre-check; this closed the TOCTOU gap.
